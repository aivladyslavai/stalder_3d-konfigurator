...
const COPING_OUTER =
  'M128.0,90.1 Q139.3,95.0 144.9,92.6 L196.3,70.4 Q201.9,68.0 190.6,63.1 L87.9,18.9 Q76.6,14.0 71.0,16.4 L19.6,38.6 Q14.0,41.0 25.3,45.9 Z'
const COPING_INNER =
...
const COPING_FILL = `${COPING_OUTER} ${COPING_INNER}`
...
export default function LoaderPoolMark({ prefix = 'lp', variant = 'loop' }) {
  ...
  const mode = variant === 'complete' ? 'is-complete' : 'is-looping'

  return (
    <svg className={`scene-loader-pool ${mode}`} viewBox="0 0 216 140" aria-hidden>
      <defs>
      ...
      <ellipse className="scene-loader-shadow" cx="108" cy="114" rx="86" ry="20" fill={`url(#${prefix}-shadow)`} />

      <g className="scene-loader-floor-g">
      ...
      <g className="scene-loader-walls">
      ...
      <g className="scene-loader-liner">
      ...
      <g className="scene-loader-water" clipPath={`url(#${clip})`}>
        ...
        <path className="scene-loader-water-sheen" fill={`url(#${sheen})`} fillRule="evenodd" d={`${WATER} ${HOLE}`} />
        <g className="scene-loader-waves">
          <g>
      ...
      <g className="scene-loader-coping">
        <path className="scene-loader-coping-fill" fillRule="evenodd" d={COPING_FILL} />
        <path className="scene-loader-coping-inner" d={COPING_INNER} />
      </g>
      <g className="scene-loader-led">
      ...
      <g className="scene-loader-stairs" clipPath={`url(#${clip})`}>
        <polygon className="scene-loader-well" points="138.7,88.7 110.7,74.8 166.7,74.8" />
        {STEPS.map(([top, front]) => (
          <g key={top} className="scene-loader-step">
      ...
      <path className="scene-loader-trace scene-loader-trace-outer" d={COPING_OUTER} />
      <path className="scene-loader-trace scene-loader-trace-inner" d={COPING_INNER} />
    </svg>
  )
}
